...
#             #     image[i][j] = [0, 0, 0]
#
#             if image[i][j][0] > 170 and image[i][j][2] < 100:
...
#     # cv2.imshow('test', hand_images[i])
#     # cv2.waitKey(500000)
#     cv2.imwrite('./1/hand' + str(i) + '.png', hand_images[i])
...
cap = cv2.VideoCapture('./MAH06243.mp4')
...
for i in range(len(hand_images)):
    ...
    # cv2.imshow('test', hand_images[i])
    # cv2.waitKey(500000)
    cv2.imwrite('./hands/hand' + str(i) + '.png', hand_images[i])
